...
async def startup():
    ...
    if deploy_model.exists():
        ...
        logger.error(
            "Deploy model expected at: %s",
            deploy_model,
        )

        logger.error(
            "Regular model expected at: %s",
            regular_model,
        )

        # Do NOT silently create an untrained
        # production inference model.
        #
        # An untrained model would produce meaningless
        # ICDAS predictions.

        engine = None

        logger.warning(
            "Inference engine disabled because "
    ...
    try:
        ...
        logger.exception(
            "Could not initialize inference engine."
        )
